...
-- text
    ...
    local fractionPart = 0 --Дробная часть от прироста
    local lastUpdate = system.getTimer() -- Время последнего пересчета количества бактерий
    
    local updateGeneration = function( event )
         if colony.owner ~= "neutral" then --Нейтральные колонии не могут генерировать войска
            local gtime = (system.getTimer() - lastUpdate)/1000 -- время прошедшее с последнего апдейта
            local newGeneration = gtime*colony.generation + fractionPart --количество бактерий сгенерированых за это время
            fractionPart = newGeneration%1 -- дробная часть сгенеррированых бактерий
            colony.bacteries.count = colony.bacteries.count + (newGeneration - fractionPart) -- к общему числу прибавляем сгененированные без дробной части
            colony:updateText() --обновляем текст
            lastUpdate = system.getTimer() --обновляем время последнего персчета
        end
    end
